...
pub fn parse_path(mut path: String) -> Result<PathBuf, String> {
    if path.starts_with(HOME_TOKEN) {
        path = path.replace(
            HOME_TOKEN,
            dirs::home_dir().ok_or("$HOME environment variable is not set")?
                .to_str()
                .ok_or("$HOME environment variable is not valid UTF-8")?,
        );
    }
